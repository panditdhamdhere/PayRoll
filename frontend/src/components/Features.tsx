    ...
    <section id="features" className="py-20">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-14">
          ...
          <p className="text-lg max-w-2xl mx-auto" style={{ color: (resolvedTheme ?? theme) === 'dark' ? 'white' : 'black' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {features.map((feature, index) => (
            <motion.div
              ...
              className="group relative rounded-2xl border border-gray-200/70 dark:border-white/10 bg-white/95 dark:bg-white/5 backdrop-blur-xl p-6 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.25)] hover:shadow-[0_28px_80px_-24px_rgba(0,0,0,0.35)] transition-all duration-300"
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-1.5" style={{ color: (resolvedTheme ?? theme) === 'dark' ? 'white' : 'black' }}>{feature.title}</h3>
                  <p style={{ color: (resolvedTheme ?? theme) === 'dark' ? 'white' : 'black' }}>{feature.description}</p>
                </div>
              </div>
